...
class DeformInputs(nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self,x):
        bs, c, h, w = x.shape
        # print('DeformInputs: h={}, w={}'.format(h, w))
        ...
        level_start_index = torch.cat((spatial_shapes.new_zeros(
            (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
        reference_points = get_reference_points([(h // 2, w // 2)], x.device)
        deform_inputs1 = [reference_points, spatial_shapes, level_start_index]
        
        spatial_shapes = torch.as_tensor([(h // 2, w // 2)], dtype=torch.long, device=x.device)
        level_start_index = torch.cat((spatial_shapes.new_zeros(
            (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
        reference_points = get_reference_points([(h, w),
                                                    (h // 2, w // 2),
                                                    (h // 4, w // 4)], x.device)
        deform_inputs2 = [reference_points, spatial_shapes, level_start_index]
...
class CrossModalExchange(nn.Module):
    def __init__(self, c, reduction=16):
        ...
        if torch.cuda.is_available():
            self.cuda()  # 确保所有子模块参数都在GPU上
    
    def forward(self, rgb, ir):
        # 输入形状: [B, N, C] (batch, sequence_length, channels)
        
        # 交换维度: [B, N, C] -> [B, C, N] 为池化做准备
        rgb_t = rgb.transpose(1, 2)  # [B, C, N]
        ir_t = ir.transpose(1, 2)    # [B, C, N]
        
        # 通道注意力权重 (在序列维度上池化)
        w_rgb = self.avg_pool(rgb_t)  # [B, C, 1]
        w_ir = self.avg_pool(ir_t)    # [B, C, 1]
        
        # 压缩维度: [B, C, 1] -> [B, C]
        w_rgb = w_rgb.squeeze(-1)
        w_ir = w_ir.squeeze(-1)
        
      
        a_rgb = self.fc(w_ir)  # IR特征指导RGB选择 [B, C]
        a_ir = self.fc(w_rgb)  # RGB特征指导IR选择 [B, C]
        
       
        a_rgb = a_rgb.unsqueeze(1)  # [B, 1, C]
        a_ir = a_ir.unsqueeze(1)    # [B, 1, C]
        
        
        rgb_out = rgb * a_rgb + rgb  # [B, N, C] * [B, 1, C] -> [B, N, C]
        ir_out = ir * a_ir + ir      # [B, N, C]
...
class DWConv(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        n = N // 21
        print('DWConv: n={}, H={}, W={}'.format(n, H, W))
        print('x',x.shape)
...
class Extractor(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
                 with_cffn=False, cffn_ratio=0.25, drop=0., drop_path=0.,
